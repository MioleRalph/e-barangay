
    <!-- Bootstrap core JavaScript-->
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/js/bootstrap.bundle.min.js"></script>

    <!-- Core plugin JavaScript-->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-easing/1.4.1/jquery.easing.min.js"></script>

    <!-- Custom scripts for all pages-->
    <script src="../components/js/sb-admin-2.min.js"></script>

    <!-- Page level plugins -->
    <script src="../components/vendor/chart.js/Chart.min.js"></script>

    <!-- Page level custom scripts -->
    <script src="../components/js/demo/chart-area-demo.js"></script>
    <script src="../components/js/demo/chart-pie-demo.js"></script>
    <!-- <script src="../components/js/demo/accounts-pie-chart.js"></script> -->

    <script src="../components/vendor/datatables/jquery.dataTables.min.js"></script>
    <script src="../components/vendor/datatables/dataTables.bootstrap4.min.js"></script>

    <!-- Page level custom scripts -->
    <script src="../components/js/demo/datatables-demo.js"></script>

</body>

</html>