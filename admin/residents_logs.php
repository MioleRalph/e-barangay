...
$residents_logs = $connection->query("SELECT * FROM `logs` WHERE `user_type` = '2' ORDER BY `timestamp` DESC")->fetchAll(PDO::FETCH_ASSOC);
        ...
        if ($delete_logs->execute([$delete_id])) {
            echo "<script>
                Swal.fire({
                    ...
                    window.location.href = 'residents_logs.php';
                });
            </script>";
        } else {
            echo "<script>
...
<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800">Residents Activity Logs</h1>
<p class="mb-4">
    Below is a comprehensive list of all resident account activities. Use the table to review, filter, or export logs for auditing and monitoring purposes.
</p>

<!-- DataTales Example -->
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h6 class="m-0 font-weight-bold text-primary">Residents Log</h6>
        <a href="../xml/export_resident_logs.php" class="btn btn-primary btn-sm">
        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped align-middle" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
                    <?php
                    $count = 1;
                    foreach ($residents_logs as $logs):
                    ?>
                        <tr>
