<?php
include '../includes/admin/admin_sidebar.php';

$history_logs = $connection->query("SELECT * FROM `resident_request_logs` ORDER BY `timestamp` DESC")->fetchAll(PDO::FETCH_ASSOC);

echo '<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>';

?>

<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800">Resident Activity Logs</h1>
<p class="mb-4">
    Below is a detailed log of all resident account activities. You can filter, search, and export the logs for auditing and record-keeping purposes.
</p>

<!-- DataTales Example -->
<div class="card shadow mb-4">


    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h6 class="m-0 font-weight-bold text-primary">
            <i class="fas fa-users"></i> Activity Logs
        </h6>
        <a href="../xml/export_admin_logs.php" class="btn btn-primary btn-sm">
            <i class="fas fa-file-export"></i> Download
        </a>
    </div>
    <div class="card-body bg-light">
        <div class="table-responsive">
            <table class="table table-hover table-striped align-middle" id="dataTable" width="100%" cellspacing="0">
                <thead class="thead-dark">
                    <tr>
                        <th>#</th>
                        <th>Account ID</th>
                        <th>Name</th>
                        <th>Activity</th>
                        <th>Activity Type</th>
                        <th>Timestamp</th>
                    </tr>
                </thead>
                <tfoot class="thead-dark">
                    <tr>
                        <th>#</th>
                        <th>Account ID</th>
                        <th>Name</th>
                        <th>Activity</th>
                        <th>Activity Type</th>
                        <th>Timestamp</th>
                    </tr>
                </tfoot>
                <tbody>
                    <?php
                    $count = 1;
                    foreach ($history_logs as $logs):
                    ?>
                        <tr>
                            <td><?php echo $count++; ?></td>
                            <td><?php echo ($logs['account_id']); ?></td>
                            <td><?php echo ($logs['name']); ?></td>
                            <td><?php echo ($logs['activity']); ?></td>
                            <td><?php echo ($logs['activity_type']); ?></td>
                            <td>
                                <span class="text-muted">
                                    <?php echo date('F j, Y g:i A', strtotime($logs['timestamp'])); ?>
                                </span>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        </div>
    </div>
</div>

<?php include '../includes/footer.php'; ?>
