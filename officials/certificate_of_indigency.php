    ...
    if (isset($_POST['approve_request'])) {
        ...
        if ($verify_approve->rowCount() > 0) {
            ...
            if (!isset($aid_request) || empty($aid_request)) {
                echo "<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Certificate of Indigency request data is not set or invalid.'
                    });
                </script>";
                exit();
            }
            ...
            if ($update_status->execute([$approve_id])) {
                ...
                foreach ($certificate_of_indigency_requests as $req) {
                ...
                $log_activity->execute([$approved_id, $account_id, $beneficiary_name, $_SESSION['full_name'], 'Request of Certificate of Indigency Approved']);

                // Insert notification for the resident
                $notification_message = "Your Certificate of Indigency request has been approved.";
                ...
                echo "<script>
                    Swal.fire({
                        icon: 'success',
                        title: 'Approved!',
                        text: 'Certificate of Indigency request has been approved successfully.',
                ...
                echo "<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        text: 'Error approving Certificate of Indigency request.',
                        showConfirmButton: false,
                        timer: 1500
                    });
                </script>";
            ...
            echo "<script>
                Swal.fire({
                    icon: 'warning',
                    title: 'Warning!',
                    text: 'Certificate of Indigency request not found!',
                    showConfirmButton: false,
                    timer: 1500
                });
            </script>";
    ...
    if (isset($_POST['reject_request'])) {
        ...
        if ($verify_reject->rowCount() > 0) {
            ...
            if ($update_status->execute([$reject_id])) {
                ...
                // Insert notification for the resident
                $notification_message = "Your Certificate of Indigency request has been rejected.";
                $insert_notification = $connection->prepare("INSERT INTO `notifications` (`resident_id`, `message`, `is_read`, `resident_type`, `created_at`) VALUES (?, ?, '0', 'null', NOW())");
                $insert_notification->execute([$account_id, $notification_message]);
                ...
                echo "<script>
                        Swal.fire({
                            icon: 'success',
                            title: 'Rejected!',
                            text: 'Certificate of Indigency request has been rejected successfully.',
                            showConfirmButton: false,
                            timer: 1500
                        }).then(() => {
                            window.location.href = 'certificate_of_indigency.php';
                ...
                echo "<script>
                        Swal.fire({
                            icon: 'error',
                            title: 'Error!',
                            text: 'Error rejecting Certificate of Indigency request.',
                            showConfirmButton: false,
                            timer: 1500
                        });
                    </script>";
