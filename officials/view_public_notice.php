...
$stmt = $connection->prepare("SELECT * FROM `announcements` WHERE `category` = 'Public Notice' ORDER BY `created_at` DESC");
$stmt->execute();
    ...
    if (isset($_POST['delete_announcement']) && $_POST['delete_announcement'] == 1) {
        ...
        if ($stmtDelete->execute([$announcementId])) {
            echo "<script>
                ...
                .then(() => { window.location.href = 'view_public_notice.php'; });
            </script>";
        } else {
